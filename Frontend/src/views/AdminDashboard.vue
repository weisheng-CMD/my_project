...
import { listTickets, createTicket, updateStatus, assignTicket, deleteTicket, getTicket } from '../api/ticket.js'
...
const form = ref({ title: '', description: '', priority: '中', category: '', creatorId: 1 })
...
async function handleCreate() {
  ...
  if (res.code === 200) {
    ...
    form.value = { title: '', description: '', priority: '中', category: '', creatorId: 1 }
    loadTickets()
  } else {
    alert(res.message)
  }
...
function openAssignDialog(ticket) {
  assignTarget.value = ticket
  assigneeId.value = ticket.assigneeId || 2
  assignDialog.value = true
}
async function handleAssign() {
  const res = await assignTicket(assignTarget.value.id, assigneeId.value)
  if (res.code === 200) {
...
async function handleDelete(id) {
  if (!confirm(`确定删除工单 #${id}？（逻辑删除）`)) return
  const res = await deleteTicket(id)
  if (res.code === 200) loadTickets()
  else alert(res.message)
}
  ...
  <div class="dashboard">
    <header class="topbar">
      ...
      <div class="topbar-right">
        <span class="user-tag">{{ auth.state.username }}（管理员）</span>
        ...
        <tbody>
          ...
          <tr v-for="t in tickets" :key="t.id">
            ...
            <td class="action-cell">
              ...
              <button class="btn btn-sm btn-outline" @click="openAssignDialog(t)">分配</button>
    ...
    <div v-if="showCreate" class="modal-overlay" @click.self="showCreate = false">
      <div class="modal">
        ...
        <div class="form-group">
          <label>创建人 ID</label>
          <input v-model.number="form.creatorId" type="number" />
        </div>
    ...
    <!-- 分配工单弹窗 -->
    <div v-if="assignDialog" class="modal-overlay" @click.self="assignDialog = false">
      <div class="modal modal-sm">
        <h2>分配工单 — #{{ assignTarget.id }}</h2>
        <p style="color:#64748b;margin-bottom:12px;">当前处理人ID：{{ assignTarget.assigneeId || '未分配' }}</p>
        <div class="form-group">
          <label>处理人 ID</label>
          <input v-model.number="assigneeId" type="number" />
        </div>
        <div class="modal-btns">
          <button class="btn btn-outline" @click="assignDialog = false">取消</button>
...
.user-tag { font-size: 13px; color: #9333ea; background: #f3e8ff; padding: 2px 10px; border-radius: 99px; }
